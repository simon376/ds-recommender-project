...
As part of the data cleaning process, the pandas columns where cleaned "on the fly" while converting the JSON data into CSV. First, the review id and book id respectively are used as the table indices. For the book information, only five of the total \todo{X} columns are used. For the remaining columns of the CSV dataset, fitting datatypes are set (instead of using all-encompassing "object" type), escaped  characters in longer texts are replaced by empty spaces.
